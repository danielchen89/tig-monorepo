...
pub async fn execute(nonce_iterators: Vec<Arc<Mutex<NonceIterator>>>, job: &Job, wasm: &Vec<u8>) {
    for nonce_iterator in nonce_iterators {
        ...
        spawn(async move {
            ...
            loop {
                let nonce = { nonce_iterator.lock().await.next() };
                let now = time();
                if nonce.is_none() || now >= job.timestamps.end {
                    break;
                }
                ...
                let skip = match job.settings.challenge_id.as_str() {
                    "c001" => {
                        type CudaSolveChallengeFn = fn(
                            &tig_challenges::c001::Challenge,
                            &Arc<CudaDevice>,
                            HashMap<&'static str, CudaFunction>,
                        ) -> anyhow::Result<
                            Option<tig_challenges::c001::Solution>,
                        >;
                        ...
                        let filter_step = match job.settings.algorithm_id.as_str() {
                            ...
                            _ => Option::<(CudaSolveChallengeFn, &Option<CudaKernel>)>::None,
                        };
                        ...
                        match filter_step {
                            Some((cuda_solve_challenge, algo_kernel)) => {
                                ...
                                let challenge =
                                    ...
                                    .unwrap();
                                match cuda_solve_challenge(&challenge, &dev, algorithm_cuda_funcs) {
                                    Ok(Some(solution)) => {
                    ...
                    "c002" => {
                        type CudaSolveChallengeFn = fn(
                            ...
                            Option<tig_challenges::c002::Solution>,
                        >;
                    ...
                    "c003" => {
                        type CudaSolveChallengeFn = fn(
                            ...
                            Option<tig_challenges::c003::Solution>,
                        >;
                    ...
                    "c004" => {
                        type CudaSolveChallengeFn = fn(
                            ...
                            Option<tig_challenges::c004::Solution>,
                        >;
                ...
                if let Ok(Some(solution_data)) = compute_solution(
                    ...
                    if verify_solution(&job.settings, nonce, &solution_data.solution).is_ok() {
